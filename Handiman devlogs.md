...
So if by example the player is on the right and the character they’re speaking to is on the right too, the player’s dialogue went off window and the other character’s dialogue went over it. Now that it is guarded by the bounds, everyone will get covered according to their anchors. Not good.

The behaviour that we want is to actually be as close as possible to the anchor, as far as possible from the screen limit, but without covering anything. To do so, we need to know where we are on screen and where is the most space available. Then, we will determine the pivot position depending on which width and height is available on each direction.
